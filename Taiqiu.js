...
(function() {
    ...
    function g() {
        ...
        for (var a = 0, e = c.Ball.balls.length; a < e; a++) {
            ...
            n.light.drawable.domDrawable.style.borderRadius =
                j + "px"
        }
    ...
    window.addEventListener('resize', function() {
        ...
        var newScreenHeight = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
        
        var newScreenRatio = newScreenWidth / newScreenHeight;
        var originalRatio = 960 / 600;
        
        // 重新计算游戏尺寸
        var newWidth, newHeight;
        if (newScreenRatio > originalRatio) {
        ...
        // 更新舞台尺寸
        if (BallGame.stage) {
            BallGame.stage.width = newWidth;
            BallGame.stage.height = newHeight;
...
(function() {
    ...
    h.prototype.move = function() {
        this.v.getLength() < 0.1 ? (this.v.x = 0, this.v.y = 0) : (this.loc.plus(this.v), this.bounce(), this.v.scale(h.scale), this.x = this.loc.x, this.y = this.loc.y, this.bitmap.setRect([this.rx, this.ry, 2 * c, 2 * c]), this.bitmap.rotation = this.v.getAngle() * 180 / Math.PI, this.rx -= this.v.getLength(), this.checkHole())
    };
    ...
    h.prototype.bounce = function() {
        ...
        if (this.loc.x < minx || this.loc.y < miny || this.loc.x > maxx || this.loc.y > maxy && !this.isDie) this.isDie = !0, this.update = this.inHole, e.Ball.type.push(this.type), console.log(this.num +
            "aaaaaaaaaaaaaaaaaaaaaaaaa lose")
    };
